...
## КРИТИЧЕСКИ ВАЖНО: ПРОВЕРКА ИСТОЧНИКА ЗАДАЧ

**ПРАВИЛО:** Проверить источник задач ПЕРЕД запуском!
...
2. **Универсальность:**
   - НЕ предполагать структуру проекта
   - НЕ предполагать нумерацию задач
...
**ВАЖНО: Логика повторного запуска воркеров (Retry Logic)**
   ...
   **Фаза 0: work_planning_task_classifier**
   1. Запустить через `task`:
      ```bash
      task '{
        "subagent_type": "work_planning_task_classifier",
        "prompt": "Классифицируй задачи из tasks.md по домену"
      }'
      ```
   2. Дождаться завершения
   3. Проверить результат

   **Фаза 0.5: work_planning_test_assigner (TDD First!)**
   1. Запустить через `task`:
      ```bash
      task '{
        "subagent_type": "work_planning_test_assigner",
      ...
      ```
   2. Дождаться завершения
   3. Проверить что создан tasks-with-test-assignments.json
   4. Проверить что TEST/CODE разделение создано

   **Фаза 1: work_planning_executor_assigner**
   1. Запустить через `task`:
      ```bash
      task '{
        "subagent_type": "work_planning_executor_assigner",
        "prompt": "Назначь исполнителей для TEST и CODE задач"
      }'
      ```
   2. Дождаться завершения
   3. Проверить что создан phase0-assignments.json
