...
2. **Фаза 2: Выполнение работы**
   ...
   - **Сгруппировать задачи по стеку технологий** (Rust, TypeScript, Python, etc.)
   - **Принцип "ПРОФЕССИОНАЛЫ ОСТАЮТСЯ":**
     - ❌ НЕ создавать агента для каждой функции/кнопки/задачи
     - ✅ Создавать агентов по домену экспертизы (frontend, backend, testing)
     - ✅ Создавать агентов по стеку технологий (rust_coder, ts_frontend, python_backend)
     - ✅ Использовать универсальных агентов для типовых задач (work_dev_specialist)
   - **Проверить существующих агентов — можно ли использовать для новых задач?**
