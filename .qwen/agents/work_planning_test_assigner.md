...
**КРИТИЧЕСКИ ВАЖНО: tasks-tdd.md — ЕДИНЫЙ ИСТОЧНИК ИСТИНЫ ПОСЛЕ СОЗДАНИЯ!**
...
## КРИТИЧЕСКИ ВАЖНО: ЕДИНЫЙ ИСТОЧНИК ИСТИНЫ

**ПРАВИЛО:** tasks-tdd.md — ЕДИНЫЙ источник истины после создания!

1. **Запрет на изменение нумерации:**
   - НЕ добавлять новые задачи между существующими
   - НЕ изменять номера существующих задач
   - Новые задачи добавлять ТОЛЬКО в конец с следующим номером

2. **Синхронизация с оркестратором:**
   - Перед запуском проверить что tasks-tdd.md существует
   - Если tasks-tdd.md НЕ существует → ОСТАНОВКА, ошибка
   - Если tasks-tdd.md существует → использовать ТОЛЬКО его

3. **Универсальность:**
   - НЕ предполагать конкретные номера задач
   - НЕ предполагать конкретные названия задач
   - Работать с ЛЮБЫМ списком задач из tasks-tdd.md
...
```
1. Прочитать tasks-tdd.md (приоритет) или tasks.md (N задач)
2. Для КАЖДОЙ задачи:
...
### Фаза 1: Чтение tasks-tdd.md (приоритет) или tasks.md
...
## Выполненная работа
- Анализ tasks-tdd.md (или tasks.md): Статус (Завершено/Неудачно/Частично)
