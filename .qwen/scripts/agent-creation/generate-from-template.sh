...
if [ -f ".qwen/scripts/validation/check-yaml-headers.sh" ]; then
    log_info "Проверка YAML заголовков..."
    bash .qwen/scripts/validation/check-yaml-headers.sh || log_warning "Обнаружены проблемы в YAML заголовке"
else
    log_warning "Скрипт проверки YAML заголовков не найден"
fi

if [ -f ".qwen/scripts/validation/check-agent-structure.sh" ]; then
    log_info "Проверка структуры агента..."
    bash .qwen/scripts/validation/check-agent-structure.sh || log_warning "Обнаружены проблемы в структуре агента"
else
