...
2. **Извлеките конфигурацию**:
   - `config.priority`: Фильтр уязвимостей по приоритету (критический, высокий, средний, низкий, все)
...
4. **КРИТИЧЕСКИ**: Поиск уязвимостей SQL-инъекций с помощью Grep:
   ```bash
...
### Фаза 3: Обнаружение XSS-уязвимостей
7. Поиск XSS-рисков с помощью Grep:
   ```bash
  ...
  "files_created": [
    {
      ...
      "reason": "Отчет об обнаружении уязвимостей",
...
Сгенерируйте исчерпывающий файл `security-scan-report.md` со следующей улучшенной структурой:
...
```bash
# Используйте навык rollback-changes (рекомендуется)
Use rollback-changes Skill with changes_log_path=.vulnerability-changes.json
